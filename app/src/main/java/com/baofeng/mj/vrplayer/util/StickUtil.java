...
public class StickUtil {
	...
	private Set<String> joystickNames = new HashSet<>();
...
//		}
	...
	public void setCallback(Activity callback){
		clearJoystickNames();
...
//			return  false;
	 ...
	 * @return true是，false不是
	 */
	public boolean isCommonHandle(String deviceName){
		if(TextUtils.isEmpty(deviceName)){
			return false;
		}
		deviceName = deviceName.toLowerCase();
	...
	public int getConnectStatus() {
		...
		if (!blutoothEnble()) {// 蓝牙关闭
			return 0;//无连接
		} else if (!isBondBluetooth()) {// 蓝牙与魔镜设备未配对
			return 0;//无连接
		} else if (!isConnected()) {// 设备未开启或者设备休眠
			return 0;//无连接
		} else {// 已连接
	...
	public Set<String> getJoystickNames() {
	 ...
	 * 获取普通手柄名
	 */
	public String getCommonHandleName() {
		for (String name : joystickNames){
			if(isCommonHandle(name)){
				return name;
			}
		}
		return "";
	 ...
	 * 获取体感手柄名
	 */
	public String getMotionHandleName() {
		for (String name : joystickNames){
			if(isMotionHandle(name)){
				return name;
			}
		}
		return "";
	...
	public void clearJoystickNames(){
		joystickNames.clear();
	}
	...
	public void removeJoystickName(String name){
		if (name != null && !name.isEmpty()) {
