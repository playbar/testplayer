...
public class GLProcessView extends GLGroupView {
    ...
    private GLImageView mPrestrainView = null;
    ...
    public void setMargin(float left, float top, float right, float bottom) {
        ...
        if(mPrestrainView != null) {
            mPrestrainView.setX(getX());
            mPrestrainView.setY(getY());
        }
...
//	}
     ...
     * @return void
     */
    public void setProcessColor(String resId) {
        if( null == mImageView) {
...
//    public void setPrestrainColor(int resId) {
//        if( null == mPrestrainView) {
//            mPrestrainView = new GLImageView(mContext);
//        }
//        mPrestrainView.setBackground(resId);
//
//        setPrestrainLayoutParams();
//    }

    private void setPrestrainLayoutParams() {
        if (mPrestrainView == null) {
            return;
        }

        this.addView(mPrestrainView);

//        if (mGlProcessMode == GLConstant.GLProcessMode.CIRCLE) {
//            mPrestrainView.setX(getX());
//            mPrestrainView.setY(getY());
//            mPrestrainView.setLayoutParams(getWidth(), getHeight());
//            if (!mRotate) {
//                mRotate = true;
//                rotate();
//            }
...
//		mProcess = process;
    ...
    private void setPrestrainProcess(float process) {
        if (process < 0 || process > 100) {
            return;
        }

        mPrestrainView.setX(getX()+getPaddingLeft());
        mPrestrainView.setY(getY()+getPaddingTop());

        mPrestrainView.setLayoutParams((getWidth() - getPaddingLeft() - getPaddingRight()) / 100 * process, getHeight() - getPaddingTop() - getPaddingBottom());

    }

    public void setPrestrainProcess(int process) {
        setPrestrainProcess((float)process);
    }
    ...
    public void setLayoutParams(float width, float height){
        ...
        mPrestrainView.setImgWH(width, height);
