...
import com.mojing.vrplayer.adapter.EpisodeAdapter;
...
public class SelectEpisodeView extends GLRelativeView{
    ...
    private ArrayList<Integer> gridDatas = new ArrayList<>();
    ...
    private void createGridView() {
        gridView = new GLGridViewScroll(mContext, 4, 5);
...
//        gridView.setOffsetY( 100);
    ...
    private void setGridListener() {
        gridView.setOnItemClickListener(new GLAdapterView.OnItemClickListener() {
            ...
            public void onItemClick(GLAdapterView<?> glparent, GLView glview, int position, long id) {
                currentNum = gridDatas.get(position);
                episodeAdapter.setNum(currentNum);
        ...
        gridView.setOnItemSelectedListener(new GLAdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(GLAdapterView<?> glparent, GLView glview, int position, long id) {
                if(gridDatas.get(position) != currentNum) {
                    GLTextView textView = (GLTextView) ((EpisodeItemView)glview).getView("textView");
                    textView.setBackground("play_bg_control_hover_150_60");
    ...
    private void handleGridData(String nums) {
        if(!TextUtils.isEmpty(nums)) {
            ...
            episodeAdapter.setData(gridDatas);

            if(gridDatas.size() > 20) {//单页数据超过20，翻页焦点可以选中
